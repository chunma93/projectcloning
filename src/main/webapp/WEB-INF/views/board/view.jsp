...
<%@ page language="java" contentType="text/html; charset=ENG"
	pageEncoding="UTF-8"%>
...
<body>
	<form method="post">
		<input type="hidden" name="pk" value="${view.pk}" /><input
			type="hidden" name="CRT_DT" /><input type="hidden" name="MOD_DT" />
		<input type="hidden" name="DEL_IF" value="${view.DEL_IF}" />
		...
		<div>
			<button type="button"
				onclick="location.href='/board/update?pk=${view.pk}'">Modify</button>
			<button type="button"
				onclick="location.href='/board/delete?pk=${view.pk}'">Delete</button>
